...
class IDRNetwork(nn.Module):
    def __init__(self, conf):
        ...
        self.fixed_sdf = bool(conf.get_int('fixed_sdf'))
        self.no_viewdir = bool(conf.get_int('no_viewdir'))
        self.no_normal = bool(conf.get_int('no_normal'))

        self.feature_vector_size = conf.get_int('feature_vector_size')
        self.implicit_network = ImplicitNetwork(self.feature_vector_size, **conf.get_config('implicit_network'))
        if self.fixed_sdf:
            self.implicit_network.requires_grad_(False)
    ...
    def get_rbg_value(self, points, view_dirs):
        if self.no_viewdir:
            view_dirs = torch.zeros_like(view_dirs)
